...
FONTb = pygame.font.SysFont(None, 32)

emoji_image = pygame.image.load("D:/tic_tac_toe/emoji.jpg")
emoji_size = 200  # Set this to the desired size
emoji_image = pygame.transform.scale(emoji_image, (emoji_size, emoji_size))

lose_image = pygame.image.load("D:/tic_tac_toe/lose.jpg")
lose_size = 200  # Set this to the desired size
lose_image = pygame.transform.scale(lose_image, (lose_size, lose_size))
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            mousX = event.pos[0] // SQUARE_SIZE
            mousY = event.pos[1] // SQUARE_SIZE
            if available_square(mousY, mousX):
                mark_square(mousY, mousX, player)
                if check_win(player):
                    game_over = True
                player = player % 2 + 1
                if not game_over:
    ...
    if not game_over:
        ...
        if check_win(1):
            draw_figures(GREEN)
            draw_lines(GREEN)
            text_surface = FONTb.render("Good Game)", True, WHITE)
            text_rect = text_surface.get_rect(center=(WIDTH // 2, HEIGHT // 2))
            screen.blit(text_surface, text_rect)
            lose_rect = lose_image.get_rect(center=(WIDTH // 2, HEIGHT // 2 + lose_size // 2 + 20))
            screen.blit(lose_image, lose_rect)
        elif check_win(2):
            draw_figures(RED)
            draw_lines(RED)
            text_surface = FONTa.render("ALHassan's AI Wins", True, WHITE)
            text_rect = text_surface.get_rect(center=(WIDTH // 2, HEIGHT // 2))
            screen.blit(text_surface, text_rect)
            emoji_rect = emoji_image.get_rect(center=(WIDTH // 2, HEIGHT // 2 + emoji_size // 2 + 20))
            screen.blit(emoji_image, emoji_rect)
